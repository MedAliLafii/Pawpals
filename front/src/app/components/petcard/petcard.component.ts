...
export class PetCardComponent implements OnInit, OnChanges {
  ...
  fetchLostPets(){
    this.isLoading = true;
    this.http.get<any[]>(`${import.meta.env.BACK_URL}/lostPet/all`)
      .subscribe(
        (data) => {
  ...
  fetchPets(): void {
    ...
    this.http.get<any[]>(`${import.meta.env.BACK_URL}/lostPet/pets?${queryParams}`)
    .subscribe(
      (response) => {
  ...
  loadLostPets() {
    this.isLoading = true;
    this.http.get<any[]>(`${import.meta.env.BACK_URL}/lostPet/lost`)
      .subscribe({
        next: (data) => {
  ...
  deleteLostPet(pet: any): void {
    console.log('Trying to delete pet with ID:', pet.lostPetID);
      ...
      this.http.delete(`${import.meta.env.BACK_URL}/lostPet/delete/${pet.lostPetID}`, {
        withCredentials: true
      }).subscribe(
      (res) => {
